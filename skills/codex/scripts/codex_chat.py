...
def resolve_codex() -> str | None:
    """Resolve the codex executable path. On Windows, shutil.which honors
    PATHEXT and picks up .cmd/.ps1/.exe shims that bare subprocess.run won't
    find."""
...
def run_codex(
    ...
    """
    # Build command — use resolved path so Windows .cmd/.ps1 shims work
    codex_path = resolve_codex() or "codex"
    cmd = [codex_path, "exec"]
